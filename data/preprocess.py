...
if __name__ == '__main__':
    ...
    for shape_dir in track(shape_list):
        ...
        # simplication
        print("original - " + str(ms.current_mesh().face_number()))
        while (ms.current_mesh().face_number() > max_faces):
            ms.apply_filter('meshing_decimation_quadric_edge_collapse', targetfacenum=max_faces, preservenormal=True)
        mesh = ms.current_mesh()
        print("simplicated - " + str(ms.current_mesh().face_number()))
